...
async def main(page: ft.Page):
    ...
    async def save_state():
        ...
        # In a real async app, data_manager would also be async
        data_manager.save_data(data)

    async def play_success_animation():
        ...
        lottie_container.on_click = lambda e: asyncio.create_task(hide_anim())
        page.update()
    ...
    async def toggle_task(task_id, value):
    ...
    def create_task_item(task):
        ...
        checkbox = ft.Checkbox(
            ...
            on_change=lambda e: asyncio.create_task(toggle_task(task["id"], e.control.value)),
        ...
        delete_btn = ft.IconButton(
            ...
            on_click=lambda e: asyncio.create_task(delete_task(task["id"]))
        )
    ...
    async def show_add_task_modal(e):
        theme = ThemeManager.get_theme(current_theme_name)
        # Update modal style
    ...
    def build_settings_drawer():
        ...
        def on_theme_click(e):
            asyncio.create_task(update_theme(e.control.data))
            # Rebuild drawer to show selection
            page.close_end_drawer()

        def on_bias_click(e):
            asyncio.create_task(update_bias(e.control.data))
            page.close_end_drawer()
        ...
        content = ft.Container(
            content=ft.Column([
                ...
                ft.Divider(color=theme.text_color),
                ft.Text("Versão 2.1 💜", size=12, color=theme.text_color, opacity=0.5, text_align=ft.TextAlign.CENTER)
    ...
    async def show_settings_modal(e):
    ...
    await update_theme(current_theme_name)
...
# Ensure assets_dir is correct for packaging
